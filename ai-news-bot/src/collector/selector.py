...
使用例::
    ...
    top_articles = select_top_articles(candidates, num=5)
"""
...
def _build_scoring_prompt(candidates: list[dict[str, Any]]) -> str:
    ...
    for i, article in enumerate(candidates):
        text = (
            f"[記事 {i}]\n"
            f"タイトル: {article.get('title', '情報なし')}\n"
            f"ソース: {article.get('source', '情報なし')}\n"
            f"概要: {article.get('summary', '情報なし')}\n"
            f"公開日時: {article.get('published_at', '情報なし')}\n"
            f"URL: {article.get('url', '情報なし')}\n"
        )
...
def select_top_articles(
    ...
    for article in selected:
        logger.info(
            ...
            article.get("title", "情報なし")[:60],
            article.get("source", "情報なし"),
        )
